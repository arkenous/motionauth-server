...
struct l2_functor {
  ...
  l2_functor(double _alpha, double _lambda) {
    alpha = _alpha;
    lambda = _lambda;
  }

  __host__ __device__ double operator()(const double& adam, const double& weight) {
    return adam - (alpha * lambda * weight);
  }
};
...
void Neuron::learn(const double delta, const vector<double> &inputValues) {
  ...
  if (this->dropout_mask == 1.0) {
    ...
    thrust::transform(d_adam_result.begin(), d_adam_result.end(), d_inputWeights.begin(), d_adam_result.begin(),
                      l2_functor(alpha, lambda));
...
unsigned long Neuron::getNumInput() {
 ...
 * @return 結合荷重
 */
double Neuron::getInputWeightIndexOf(const unsigned long i) {
...
double Neuron::getMIndexOf(const unsigned long i) {
  return this->h_m[i];
}

double Neuron::getNuIndexOf(const unsigned long i) {
  return this->h_nu[i];
}
